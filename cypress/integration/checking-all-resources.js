describe('checking all resources are loaded with content', function () {
  ...
  it('Select en_sq File', function () {
    /** Select file */
    // cy.contains('Study Questions in TSV files for Bible books').click();
    cy.contains('sq_TIT.tsv').click()
    // .then(() => {
    let english;
    let message = 'hello';
    if (cy.contains('unfoldingWord')) {
      english = {
        greeting() {
          return message
        },
    ...
    cy.wrap(english).invoke('greeting').should('equal', 'The organisation does not contain the selected translation english for the repository')
    // })
    // .then(($sq_TIT.tsv) => {

    // })
    // cy.wait(2000);
    // cy.contains('No organizations found for this account')

    // cy.wrap(english).invoke('greeting').should('eq', 1, { message: 'custom error message 1' })

    // cy.contains('Titus Study Questions');
    // cy.contains('OK').should('be.be.visible').click();
